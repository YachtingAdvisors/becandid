    ...
    return (
      <div className="max-w-2xl mx-auto">
        <div className="card p-12 text-center">
          ...
          <h2 className="font-display text-2xl font-semibold text-ink mb-3">No Active Partnership</h2>
          <p className="text-sm text-ink-muted mb-6">When someone invites you as their accountability partner, this is where you'll see their progress.</p>
    ...
    <div className="max-w-3xl mx-auto space-y-6 stagger">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="card px-4 py-3 text-center">
          <div className="text-2xl font-display font-bold text-brand-600">{data.balance.toLocaleString()}</div>
          <div className="text-xs text-ink-muted mt-0.5">Trust Points</div>
        </div>
        <div className="card px-4 py-3 text-center">
          <div className="text-2xl font-display font-bold text-emerald-600">{data.streak.streakDays}</div>
          <div className="text-xs text-ink-muted mt-0.5">Day Streak</div>
        </div>
        <div className="card px-4 py-3 text-center">
          <div className={`text-2xl font-display font-bold ${actionCount > 0 ? 'text-amber-500' : 'text-emerald-600'}`}>
            {actionCount > 0 ? actionCount : '✓'}
          </div>
          <div className="text-xs text-ink-muted mt-0.5">{actionCount > 0 ? 'Action Needed' : 'All Clear'}</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <Link href="/partner/focus" className="card p-5 hover:shadow-md transition-shadow text-center">
          ...
          <div className="font-display text-sm font-semibold text-ink">Focus Board</div>
          <p className="text-xs text-ink-muted mt-1">3-week heatmap & milestones</p>
        </Link>
        <Link href="/partner/checkins" className="card p-5 hover:shadow-md transition-shadow text-center">
          <div className="text-3xl mb-2">📋</div>
          <div className="font-display text-sm font-semibold text-ink">Check-ins</div>
        ...
        <Link href="/partner/conversations" className="card p-5 hover:shadow-md transition-shadow text-center">
          <div className="text-3xl mb-2">💬</div>
          <div className="font-display text-sm font-semibold text-ink">Conversations</div>
          <p className="text-xs text-ink-muted mt-1">
        ...
        <Link href="/partner/encourage" className="card p-5 hover:shadow-md transition-shadow text-center">
          <div className="text-3xl mb-2">💪</div>
          <div className="font-display text-sm font-semibold text-ink">Encourage</div>
          <p className="text-xs text-ink-muted mt-1">Send a supportive message</p>
        </Link>
      </div>
    </div>
  );
