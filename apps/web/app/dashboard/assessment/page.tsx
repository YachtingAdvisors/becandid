...
/* ─── Scoring ────────────────────────────────────────────── */
function calculateResults(selected: Set<string>): { id: RivalId; label: string; icon: string; color: string; desc: string; pct: number }[] {
      ...
      <div className="text-center space-y-3 pt-4">
        ...
        <div className="flex items-center justify-center pt-2">
          <button
            ...
            className="relative group flex items-center gap-0 rounded-full p-1 bg-surface-container ring-1 ring-outline-variant/20 cursor-pointer transition-all duration-300 hover:ring-primary/30"
          >
            <div
            ...
            <span
              className={`relative z-10 flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-label font-bold transition-colors duration-300 ${
                !showGrowth ? 'text-white' : 'text-on-surface-variant'
              }`}
            >
          ...
          <div
            ...
            className={`relative overflow-hidden rounded-2xl ring-1 transition-all duration-500 ${
              showGrowth
                ...
                : 'ring-outline-variant/10 bg-surface-container-lowest'
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] font-label font-bold uppercase tracking-wider ${
                      showGrowth ? 'text-emerald-600' : 'text-on-surface-variant/60'
                  ...
                  <h3 className="font-headline font-extrabold text-lg text-on-surface mt-0.5 tracking-tight">
                    {verb}
                  </h3>
                </div>
                <span className={`font-headline font-bold text-base tabular-nums ${
                  showGrowth ? 'text-emerald-600' : (item.pct >= 60 ? 'text-red-500' : 'text-on-surface-variant')
                } transition-colors duration-500`}>
                  {item.pct}%
                </span>
              </div>
              ...
              {showGrowth && (
                <div className="flex items-start gap-2.5 bg-emerald-500/5 border border-emerald-500/10 rounded-xl p-3.5 transition-all duration-500">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-on-surface font-body leading-relaxed">{t.climbPractice}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            return (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-headline font-bold text-xs text-on-surface truncate">{verb}</p>
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        {results.length === 0 ? (
          ...
          <div className="space-y-3">
            {(showAll ? results : results.slice(0, 7)).map((rival, idx) => {
              const isExpanded = expandedId === rival.id;
              return (
                <div
                  key={rival.id}
                  className={`bg-surface-container-lowest rounded-2xl ring-1 ring-outline-variant/10 p-4 transition-all duration-200 cursor-pointer ${
                    idx < 3 ? 'shadow-md' : ''
                  }`}
                ...
                >
                  ...
                  {isExpanded && (
                    <p className="pl-[4.5rem] pt-3 text-sm text-on-surface-variant font-body leading-relaxed">
                      {rival.desc}
                    </p>
                  )}
                </div>
            ...
            {results.length > 7 && (
              <button
                ...
                className="w-full flex items-center justify-center gap-1.5 py-3 text-sm font-label font-semibold text-primary hover:text-primary/80 transition-colors cursor-pointer"
              >
                <span className="material-symbols-outlined text-base">
                  {showAll ? 'expand_less' : 'expand_more'}
                </span>
                {showAll ? 'Show less' : `Show ${results.length - 7} more`}
              </button>
            )}
          </div>
        )}
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between">
              <h3 className="font-headline font-bold text-sm text-on-surface">{meta.label}</h3>
              {pct !== undefined && (
                <span className={`font-headline font-bold text-xs ${pct >= 70 ? 'text-red-500' : pct >= 45 ? 'text-amber-500' : 'text-on-surface-variant'}`}>{pct}%</span>
              )}
            </div>
            {growth && (
              <div className="flex items-center gap-1 mt-1">
                <span className="text-[10px] text-on-surface-variant font-label">{growth.slide}</span>
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <p className="text-sm text-on-surface-variant font-label">
            <span className="font-bold text-on-surface">{chosenRivals.size}</span> selected
          </p>
          {chosenRivals.size >= 6 && (
        ...
        {results.length > 0 && (
          <div>
            <p className="text-xs font-label font-semibold text-on-surface-variant uppercase tracking-wider mb-2.5">From your assessment</p>
        ...
        {/* All other rivals */}
        <div>
          <p className="text-xs font-label font-semibold text-on-surface-variant uppercase tracking-wider mb-2.5">All rivals</p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        {/* Actions */}
        <div className="flex flex-col items-center gap-3 pt-4 sticky bottom-0 bg-gradient-to-t from-background via-background to-transparent pb-2">
          <button
