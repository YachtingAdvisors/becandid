  ...
  return (
    <div className="space-y-3">
          ...
          >
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-ink-muted mt-0.5 leading-snug">{group.description}</p>
              </div>
            ...
            {isExpanded && (
              <div className="px-3 pb-3 space-y-1.5 border-t border-surface-border/50 pt-2">
                    ...
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-all duration-150 ${
                        isSelected
                          ...
                          : 'bg-surface-muted/50 border-2 border-transparent hover:bg-surface-muted hover:border-brand-200'
                      } disabled:opacity-50`}
                    >
                      <span className="text-lg flex-shrink-0">{getCategoryEmoji(cat)}</span>
                      <div className="flex-1 min-w-0">
                        <div className="text-sm font-medium text-ink">{GOAL_LABELS[cat]}</div>
              ...
              <div className="px-4 pb-3 flex flex-wrap gap-1.5">
                {group.categories
                  ...
                  .map(cat => (
                    <span
                      key={cat}
                      className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-brand-100 text-brand-700 text-[11px] font-medium"
                    >
      ...
      <button
        ...
        className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-2xl border-2 text-left transition-all duration-200 ${
          selected.includes('custom')
            ? 'border-brand-400 bg-brand-50/30'
            : 'border-dashed border-surface-border hover:border-brand-200'
        } disabled:opacity-50`}
      >
        <span className="text-2xl">⚙️</span>
        <div className="flex-1">
          <div className="text-sm font-semibold text-ink">Custom</div>
          <p className="text-xs text-ink-muted mt-0.5">Define your own category to monitor</p>
        </div>
        {selected.includes('custom') && (
          <div className="w-5 h-5 rounded-full bg-brand-600 flex items-center justify-center flex-shrink-0">
            <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
      ...
      <div className="text-center pt-1">
        <span className={`text-sm font-medium ${selected.length > 0 ? 'text-brand-600' : 'text-ink-muted'}`}>
          {selected.length === 0
