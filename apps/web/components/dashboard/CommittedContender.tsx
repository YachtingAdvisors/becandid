    ...
    <div className="space-y-4">
      ...
      {showImpact && (
        <div className="card p-5 space-y-4 animate-fade-in">
          <h3 className="text-sm font-semibold text-ink">How are you doing right now?</h3>
          <p className="text-xs text-ink-muted">This is for you. You choose whether your partner sees it.</p>

          {/* Feelings */}
          <div>
            <p className="text-xs font-medium text-ink mb-2">What are you feeling? <span className="text-ink-muted">(select all that apply)</span></p>
          ...
          {/* Trust meter */}
          <div>
            <p className="text-xs font-medium text-ink mb-2">Trust level right now</p>
          ...
          {/* Safety */}
          <div>
            <p className="text-xs font-medium text-ink mb-2">Do you feel safe in this relationship?</p>
                ...
                <button key={String(opt.v)} onClick={() => setFeelsSafe(feelsSafe === opt.v ? null : opt.v)}
                  className={`flex-1 py-2.5 rounded-lg text-sm font-medium border transition-all ${
                    feelsSafe === opt.v
                      ...
                      : 'border-surface-border text-ink-muted hover:bg-gray-50'
                  }`}>
                  {opt.label}
                </button>
              ))}
          ...
          {/* Reflection */}
          <div>
            <p className="text-xs font-medium text-ink mb-2">Anything else on your heart? <span className="text-ink-muted">(optional)</span></p>
            <textarea value={reflection} onChange={(e) => setReflection(e.target.value)}
              placeholder="Whatever you need to say…"
              className="w-full h-20 px-3 py-2.5 rounded-lg border border-surface-border bg-white text-ink text-sm resize-none focus:outline-none focus:ring-2 focus:ring-brand/20 placeholder:text-ink-muted/50" />
          </div>

          {/* Consent toggle */}
          <div className="flex items-center justify-between py-2">
            <div>
              <p className="text-xs font-medium text-ink">Share with your partner?</p>
              <p className="text-[10px] text-ink-muted">They'll see your feelings and trust level — not your reflection</p>
            </div>
            <button onClick={() => setShareWithPartner(!shareWithPartner)}
              className={`relative w-10 h-5 rounded-full transition-colors ${shareWithPartner ? 'bg-brand' : 'bg-gray-200'}`}>
      ...
      {trustHistory.length > 1 && (
        <div className="card p-5">
          <h3 className="text-sm font-semibold text-ink mb-3">Your Trust Journey</h3>
      ...
      <div className="card p-4 bg-gradient-to-r from-violet-50 to-rose-50 border-violet-100">
        <div className="flex items-center gap-3">
          ...
          <a href="/partner/journal"
            className="shrink-0 px-3 py-1.5 text-xs font-medium rounded-lg bg-brand text-white hover:bg-brand-dark">
            Write
          </a>
        </div>
      </div>
