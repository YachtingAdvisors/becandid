...
const MOODS: { key: Mood; icon: string; label: string; idleBg: string; hoverBg: string; color: string }[] = [
  {
    key: 'great',
    icon: 'sentiment_very_satisfied',
    label: 'Great',
    idleBg: 'bg-tertiary-container/20',
  ...
  },
  {
    ...
    label: 'Calm',
    idleBg: 'bg-surface',
    hoverBg: 'hover:bg-surface-container',
    color: 'text-primary',
  },
  {
    key: 'okay',
  ...
  },
  {
    ...
    label: 'Low',
    idleBg: 'bg-error-container/20',
    hoverBg: 'hover:bg-error-container/40',
    color: 'text-error',
  },
];
...
export default function QuickMoodCheckin() {
      ...
      <div className="bg-surface-container-lowest rounded-[2rem] p-8">
        ...
        {selected === 'crisis' && (
          <div className="mt-4 space-y-3">
            ...
            <a
              href="tel:988"
              className="flex items-center gap-3 p-3 bg-error/5 rounded-2xl ring-1 ring-error/10 hover:ring-error/30 transition-all duration-200 cursor-pointer"
            >
  ...
  if (state === 'nudge-prompt' && selected) {
    ...
    return (
      <div className={`bg-surface-container-lowest rounded-[2rem] ring-1 ${ringColor} p-8`}>
        ...
        {showMessageInput ? (
          <div className="mb-4">
            <textarea
              ...
              maxLength={500}
              rows={2}
              className="w-full px-3 py-2 text-sm font-body text-on-surface bg-surface rounded-2xl ring-1 ring-outline-variant/20 focus:ring-primary/40 focus:outline-none resize-none placeholder:text-on-surface-variant/50"
            />
        ...
        <div className="flex gap-2">
          <button
            onClick={handleNudge}
            className={`flex-1 py-3 rounded-2xl font-label text-sm font-bold text-white ${btnBg} cursor-pointer transition-all duration-200 flex items-center justify-center gap-2`}
          >
          ...
          <button
            ...
            className="flex-1 py-3 rounded-2xl font-label text-sm text-on-surface-variant bg-surface hover:bg-surface-container cursor-pointer transition-all duration-200"
          >
            I&apos;m okay for now
          </button>
        </div>
  ...
  if (state === 'submitted' && selected) {
      ...
      return (
        <div className="bg-surface-container-lowest rounded-[2rem] p-8">
          <div className="flex items-center justify-center gap-2 py-4">
            <MaterialIcon name="check_circle" filled className="text-tertiary text-2xl" />
    ...
    if (selected === 'crisis') {
      return (
        <div className="bg-surface-container-lowest rounded-[2rem] ring-1 ring-error/20 p-8">
          <div className="text-center mb-4">
        ...
        <div className="flex gap-3">
          {MOODS.map((mood) => (
            <button
              ...
              disabled={cooldown}
              className={`flex-1 py-4 rounded-2xl flex flex-col items-center gap-2 cursor-pointer transition-all duration-200 ${mood.hoverBg} disabled:opacity-40 disabled:cursor-not-allowed ${
                selected === mood.key
                  ? 'ring-2 ring-primary bg-primary/[0.08]'
                  : mood.idleBg
              }`}
            >
              <MaterialIcon name={mood.icon} filled className={`text-3xl ${mood.color}`} />
